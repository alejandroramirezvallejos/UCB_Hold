...
public class EstadoPrestamoJob
{
    ...
    public async Task Execute()
    {
        ...
        var atrasadosIds = await _db.Prestamos
        ...
        {
            await _db.Prestamos
                .Where(p => atrasadosIds.Contains(p.Id))
                .ExecuteUpdateAsync(s => s.SetProperty(p => p.EstadoPrestamo, EstadoPrestamo.Atrasado));

            foreach (var id in atrasadosIds)
                await _audit.Log(AuditAccion.AtrasadoAutomatico, nameof(PrestamoEntity), id.ToString());
        }

        var rechazadosIds = await _db.Prestamos
        ...
        {
            await _db.Prestamos
                .Where(p => rechazadosIds.Contains(p.Id))
