...
public class CarritoController : ControllerBase
{
    ...
    public async Task<IActionResult> GetUnavailableDates([FromBody] CarritoDto request)
    {
        var result = await _service.GetUnavailableDates(request.FechaInicio, request.FechaFin, request.Carrito);

        return result.IsSuccess ? Ok(new Response<List<CarritoDto>> { Status = 200, Value = result.Value }) : BadRequest(new Response<object> { Status = 400, Errors = result.Errors.ToList() });
    }

    [HttpPost("disponibilidadEquipos")]
    public async Task<IActionResult> GetAvailability([FromBody] CarritoDto request)
    {
