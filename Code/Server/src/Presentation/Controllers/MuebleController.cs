...
public class MuebleController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        return result.IsSuccess
            ? Ok(new Response<List<MuebleDto>> { Status = 200, Value = result.Value })
    ...
    public async Task<IActionResult> Get(int id)
    {
        ...
        return result.IsSuccess
            ? Ok(new Response<MuebleDto> { Status = 200, Value = result.Value })
    ...
    public async Task<IActionResult> Create([FromBody] MuebleDto dto)
    {
        ...
        return result.IsSuccess
            ? CreatedAtAction(nameof(Get), new { id = result.Value?.Id }, new Response<MuebleDto> { Status = 201, Value = result.Value })
            : BadRequest(new Response<object> { Status = 400, Errors = result.Errors.ToList() });
    }
