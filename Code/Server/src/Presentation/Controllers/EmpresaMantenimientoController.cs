...
public class EmpresaMantenimientoController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        return result.IsSuccess
            ? Ok(new Response<List<EmpresaMantenimientoDto>> { Status = 200, Value = result.Value })
    ...
    public async Task<IActionResult> Get(int id)
    {
        ...
        return result.IsSuccess
            ? Ok(new Response<EmpresaMantenimientoDto> { Status = 200, Value = result.Value })
    ...
    public async Task<IActionResult> Create([FromBody] EmpresaMantenimientoDto dto)
    {
        ...
        return result.IsSuccess
            ? CreatedAtAction(nameof(Get), new { id = result.Value?.Id }, new Response<EmpresaMantenimientoDto> { Status = 201, Value = result.Value })
            : BadRequest(new Response<object> { Status = 400, Errors = result.Errors.ToList() });
    }
