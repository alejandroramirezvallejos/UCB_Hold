...
public class MantenimientoController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        return result.IsSuccess
            ? Ok(new Response<List<MantenimientoDto>> { Status = 200, Value = result.Value })
    ...
    public async Task<IActionResult> Get(int id)
    {
        ...
        return result.IsSuccess
            ? Ok(new Response<MantenimientoDto> { Status = 200, Value = result.Value })
    ...
    public async Task<IActionResult> Create([FromBody] MantenimientoDto dto)
    {
        ...
        return result.IsSuccess
            ? CreatedAtAction(nameof(Get), new { id = result.Value?.Id }, new Response<MantenimientoDto> { Status = 201, Value = result.Value })
            : BadRequest(new Response<object> { Status = 400, Errors = result.Errors.ToList() });
    }
