...
public class CarreraController : ControllerBase
{
    ...
    public CarreraController(CarreraService service) => _service = service;
    ...
    public async Task<IActionResult> GetAll()
    {
        ...
        return result.IsSuccess
            ? Ok(new Response<List<CarreraDto>> { Status = 200, Value = result.Value })
    ...
    public async Task<IActionResult> Get(int id)
    {
        ...
        return result.IsSuccess
            ? Ok(new Response<CarreraDto> { Status = 200, Value = result.Value })
    ...
    public async Task<IActionResult> Create([FromBody] CarreraDto dto)
    {
        ...
        return result.IsSuccess
            ? CreatedAtAction(nameof(Get), new { id = result.Value?.Id }, new Response<CarreraDto> { Status = 201, Value = result.Value })
            : BadRequest(new Response<object> { Status = 400, Errors = result.Errors.ToList() });
    }
