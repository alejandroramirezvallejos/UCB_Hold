...
public class ComponenteService : Service<ComponenteEntity, ComponenteRepository, ComponenteDto>
{
    public ComponenteService(
        ...
        ComponenteMapper mapper,
        IValidator<ComponenteDto> validator,
        AuditLogService audit
    )
        : base(repository, validator, mapper, audit) { }

    public override async Task<Result<ComponenteDto>> Create(ComponenteDto dto)
    {
        await ResolveEquipoId(dto);
        return await base.Create(dto);
    }

    public override async Task<Result<ComponenteDto>> Update(int id, ComponenteDto dto)
    {
        await ResolveEquipoId(dto);
    ...
    private async Task ResolveEquipoId(ComponenteDto dto)
    {
        ...
        if (
            !string.IsNullOrWhiteSpace(dto.CodigoImtEquipo)
