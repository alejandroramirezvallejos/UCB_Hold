...
{
    private static readonly string[] ValidosEstados = ["operativo", "parcialmente_operativo", "inoperativo"];
    ...
    public EquipoValidator(ApplicationDbContext dbContext)
    {
        RuleFor(e => e.IdGrupoEquipo)
            ...
            .MustAsync(async (id, cancellationToken) => await dbContext.GruposEquipos.AnyAsync(g => g.Id == id && !g.EstadoEliminado, cancellationToken))
            .WithMessage("Grupo equipo no existe");

        RuleFor(e => e.IdGavetero)
            .MustAsync(async (id, cancellationToken) => await dbContext.Gaveteros.AnyAsync(g => g.Id == id && !g.EstadoEliminado, cancellationToken))
        ...
        RuleFor(e => e.EstadoEquipo)
            .Must(estado => string.IsNullOrEmpty(estado)
                || ValidosEstados.Contains(estado.ToLowerInvariant()))
